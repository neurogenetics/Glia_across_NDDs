...
DIR=/data/ADRD/glia_across_NDDs/analysis/microglia/cluster_characterization/cNMF/outs
...
~/.local/bin/cnmf consensus --output-dir $DIR --name microglia --components 13 --local-density-threshold 2 --show-clustering
~/.local/bin/cnmf consensus --output-dir $DIR --name microglia --components 35 --local-density-threshold 2 --show-clustering

# then run again after checking density threshold plot
~/.local/bin/cnmf consensus --output-dir $DIR --name microglia --components 13 --local-density-threshold 0.28 --show-clustering
~/.local/bin/cnmf consensus --output-dir $DIR --name microglia --components 35 --local-density-threshold 0.16 --show-clustering
