...
oligo_celllevel_meta <- readRDS("./analysis/oligodendrocytes/oligos_celllevel_metadata_ANNOTATED.rds")
...
oligo_celllevel_meta$donor_region <- paste0(oligo_celllevel_meta$donor, "_", oligo_celllevel_meta$region)
t <- as.data.frame(table(oligo_celllevel_meta$cluster_anno, oligo_celllevel_meta$donor_region))
...
saveRDS(props, "./analysis/oligodendrocytes/cluster_proportions/oligo_raw_donor_region_props.rds")

########################################
########################################
########################################

# GLM analysis of oligoglia proportions w/ logit transformation

########################################

oligo_props <- readRDS("./analysis/oligodendrocytes/cluster_proportions/oligo_raw_donor_region_props.rds")
donor_meta <- readRDS("./metadata/cleaned/meta_merged.rds")


oligo_props$dataset_region <- paste0(oligo_props$study, "_", oligo_props$region)
dataset_regions <- unique(oligo_props$dataset_region)
...
for (dataset_region in dataset_regions){
  t <- oligo_props[oligo_props$dataset_region == dataset_region, ]
...
oligo_glm_summary_combined <- overall_GLM_summary %>%
  filter_all(all_vars(!is.infinite(.))) %>%
  mutate(p_adj = p.adjust(`Pr(>|t|)`, method = "fdr"))

oligo_glm_summary_combined <- oligo_glm_summary_combined[!oligo_glm_summary_combined$`Pr(>|t|)` == "NaN", ]

saveRDS(oligo_glm_summary_combined, file = "./analysis/oligodendrocytes/cluster_proportions/oligo_props_logit_GLM_results_1v1.rds")
