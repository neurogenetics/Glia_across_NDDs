...
props_mashr <- readRDS("./analysis/oligodendrocytes/cluster_proportions/oligo_props_mashr_obj_canonical.rds")
props_raw <- readRDS("./analysis/oligodendrocytes/cluster_proportions/oligo_raw_donor_region_props.rds")
...
# 
# png('./analysis/oligodendrocytes/plots_final/props_mashr_corrplot.png', res = 600, width = 10, height = 8, units = "in")
...
annotation_data$region <- ifelse(grepl("HC", annotation_data$region), "HIP", annotation_data$region)
...
colnames(postmean) <- ifelse(grepl("AD_HC", colnames(postmean)), "Mathys_2024_AD_HIP",
                             ifelse(grepl("CR_HC", colnames(postmean)), "Mathys_2024_CR_HIP", colnames(postmean)))
...
png('./analysis/oligodendrocytes/plots_final/props_heatmap.png', res = 600, width = 14.5, height = 5.5, units = "in")
...
plot_props <- function(props_df, lfsr_df = NULL, dataset, clusters, region, disease, title){
  ...
  strip_colors <- oligo_colors[cluster_levels]
...
p1 <- plot_props(props_df = props_raw, lfsr_df = lfsr_df, dataset = "Mathys", 
                 clusters = c("Oligo_Stress_HSPH1", "Oligo_OPALIN"), 
                 region = "EC", disease = "AD", title = "Mathys 2024, EC, AD vs. HC")

p2 <- plot_props(props_df = props_raw, lfsr_df = lfsr_df, dataset = "Pineda", 
                 clusters = c("Oligo_Stress_HSPH1", "Oligo_OPALIN"), 
                 region = "PFC", disease = "C9FTLD", title = "Pineda 2024, PFC, C9FTLD vs. HC") +
  theme(axis.title.y = element_blank())

p3 <- plot_props(props_df = props_raw, lfsr_df = lfsr_df, dataset = "AMP-PD", 
                 clusters = c("Oligo_Stress_HSPH1", "Oligo_OPALIN"), 
                 region = "M1", disease = "PD", title = "NM 2024, M1, PD vs. HC") +
  theme(axis.title.y = element_blank())


p4 <- plot_grid(p1, p2, p3, nrow = 1)

svglite("./analysis/oligodendrocytes/plots_final/raw_props_boxplots.svg", width = 15, height = 3.5)
plot(p4)
dev.off()
