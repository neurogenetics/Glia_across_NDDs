...
tPCA_lfsrs <- readRDS("./analysis/oligodendrocytes/differential_expression/oligodendrocytes_mashr_tPCA_lfsrs.rds")
...
pl = data.frame(mi = get_estimated_pi(oligos_mashr))
...
ggsave(plot = p1, filename = "./analysis/oligodendrocytes/plots_final/diffexp_mashr_mixtures.png", width = 9.5, height = 5.5, dpi = 600)
...
tPCA_corr <- oligos_mashr$fitted_g$Ulist$ED_tPCA
...
png('./analysis/oligodendrocytes/plots_final/diffexp_mashr_corrplot.png', res = 600, width = 10, height = 8, units = "in")
...
png('./analysis/oligodendrocytes/plots_final/diffexp_hmp.png', res = 600, width = 11.5, height = 6.5, units = "in")
draw(hmp1)
dev.off()
...
gobp_c2 <- readRDS("./analysis/oligodendrocytes/differential_expression/oligodendrocytes_gobp_c2.rds")
gobp_c3 <- readRDS("./analysis/oligodendrocytes/differential_expression/oligodendrocytes_gobp_c3.rds")
...
# cluster 3
gobp_plot <- gobp_c3[1:5,]
gobp_plot$`-log10(FDR)` <- -log10(gobp_plot$p.adjust)
gobp_plot <- gobp_plot %>%
  arrange(desc(`-log10(FDR)`))
gobp_plot$Description <- c(gobp_plot$Description[1], 
                           "protein localization to site\nof double-strand break", 
                           gobp_plot$Description[3:4],
...
svglite(filename = "./analysis/oligodendrocytes/plots_final/GOBP_c2.svg", width = 7, height = 3.2)
plot(p1)
dev.off()

svglite(filename = "./analysis/oligodendrocytes/plots_final/GOBP_c3.svg", width = 7, height = 3.2)
plot(p2)
dev.off()
