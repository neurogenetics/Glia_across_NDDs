...
nebula_results <- list.files("./analysis/astrocytes/differential_expression/nebula_results/", pattern = "\\.rds$", full.names = T)
nebula_results <- lapply(nebula_results, readRDS)
nebula_names <- list.files("./analysis/astrocytes/differential_expression/nebula_results/", pattern = "\\.rds$", full.names = F)
nebula_names <- gsub("_nebula_results.rds", "", nebula_names)
names(nebula_results) <- nebula_names


DESeq_results <- list.files("./analysis/astrocytes/differential_expression/DESeq_results/", pattern = "\\.rds$", full.names = T)
DESeq_results <- lapply(DESeq_results, readRDS)
DESeq_names <- list.files("./analysis/astrocytes/differential_expression/DESeq_results/", pattern = "\\.rds$", full.names = F)
...
saveRDS(merged_results_list, file = "./analysis/astrocytes/differential_expression/astrocytes_diffexp_results_for_mashr.rds")
