...
# (for testing) fastq <- "D19-8398"

####################################

demuxafy_combined_results_dir <- paste0("/data/ADRD/AD_multiregion/fastq_processing/demuxafy/outs/", fastq, "/combined/combined_results.tsv")
...
cellbender_barcodes_dir <- paste0("/data/ADRD/AD_multiregion/fastq_processing/cellbender/outs/", fastq, "/cellbender_cell_barcodes.csv")
...
cellranger_mat_dir <- paste0("/data/ADRD/AD_multiregion/fastq_processing/cellranger/outs/", fastq, "/raw_feature_bc_matrix/")
...
cellbender_mat_dir <- paste0("/data/ADRD/AD_multiregion/fastq_processing/cellbender/outs/", fastq, "/cellbender_filtered.h5")
...
meta <- read.csv(file = "/data/ADRD/AD_multiregion/fastq_processing/AD_basic_metadata.csv", header = T)
meta <- meta[meta$library_id %in% fastq, ]
region <- meta$region
sample <- substring(meta$subject, 1, 5)
...
cellranger_seurat_dir <- paste0("/data/ADRD/AD_multiregion/fastq_processing/final_outs/", fastq, "/", sample, "_", region, "_cellranger_seurat.rds")
cellbender_seurat_dir <- paste0("/data/ADRD/AD_multiregion/fastq_processing/final_outs/", fastq, "/", sample, "_", region, "_cellbender_seurat.rds")
